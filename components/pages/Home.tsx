...
export function HomePage({ locale }: { locale: Locale }) {
  ...
  for (const c of homeCats) {
    productsByCat[c.id] = getProductsByCategory(c.id, locale).slice(0, 4);
  }
  // Ensure every category tab shows something even if seeded thinly.
  const fallback = getAllProducts(locale).slice(0, 4);
  for (const c of homeCats) {
  ...
  return (
    <>
      ...
      <section className="relative overflow-hidden bg-navy lg:min-h-[600px] lg:bg-white">
        ...
        <Container className="relative z-10 py-14 lg:py-[100px] lg:pb-[120px]">
          <div className="max-w-[600px] text-white">
            <Eyebrow tone="yellow">{t.home.heroEyebrow}</Eyebrow>
            <h1 className="mt-5 text-[42px] font-extrabold leading-[1.0] tracking-[-0.02em] sm:text-6xl lg:text-[68px]">
              {t.home.heroTitleA}
              <br />
              <span className="text-yellow">{t.home.heroTitleAccent}</span>{" "}
              {t.home.heroTitleB}
            </h1>
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              <Button href={localeHref(locale, "/products")} kind="primary" size="lg">
                {t.home.ctaCatalog}
              </Button>
              <Button
      ...
      <section className="bg-bg py-20">
        <Container>
          <div className="mb-8 flex flex-wrap items-end justify-between gap-4">
            ...
            >
              {t.home.industriesAll} <Icon name="arrow" size={14} />
            </a>
          </div>
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {industries.map((ind) => (
              <IndustryTile key={ind.id} industry={ind} locale={locale} />
            ))}
          </div>
        </Container>
      </section>
      ...
      <section className="border-y border-line bg-bg-alt py-20">
        <Container>
          <SectionHead
            eyebrow={t.home.catsEyebrow}
            title={t.home.catsTitle}
            sub={t.home.catsSub}
            className="mb-7"
          />
          <CategoryTabs cats={tabCats} productsByCat={productsByCat} locale={locale} />
        </Container>
      </section>
      ...
      <section className="bg-navy text-white">
        <Container className="grid items-center gap-14 py-[60px] lg:grid-cols-[1.4fr_1fr]">
          <div>
            ...
            <p className="mt-4 max-w-[540px] text-base text-white/85">{t.home.contactSub}</p>
          </div>
