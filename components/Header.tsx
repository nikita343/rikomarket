...
import { localeHref, splitLocale, type Locale } from "@/lib/i18n";
...
export function Header({ locale }: { locale: Locale }) {
  ...
  const { nav } = getSite(locale);

  // Compare against the locale-independent part of the path.
  const current = splitLocale(pathname ?? "/").rest;
